...
class NsGastroEvent
{
    ...
    public static function registerFooterScript(Output $output)
    {
        $output->addView('NsGastro::pos.footer', [
            'gastroJsSrc'    =>  is_file( base_path( 'modules/NsGastro/.dev' ) ) ? 
                self::loadFilesFromMix( '/js/Gastro.js' ) : 
                self::loadFilesFromMix( '/js/Gastro.min.js' ),
            'gastroCssSrc'    =>  self::loadFilesFromMix( '/css/gastro.css' ),
        ]);
    }

    public static function loadFilesFromMix( $file )
    {
        $filePath   =   base_path( 'modules/NsGastro/Public/mix-manifest.json' );
        ...
        if ( is_file( $filePath ) ) {
            ...
            if ( isset( $decoded[ $file ] ) ) {
                return asset( 'modules/nsgastro/' . $decoded[ $file ] );
            }
        }

        throw new NotFoundException( sprintf( __( 'Unable to locate the file %s' ), $file ) );
    }
    ...
    public static function proceedFreedingTable($order)
    {
        ...
        if (
            ...
            if ($totalPaid === $session->orders->count()) {
                ...
                TableAfterUpdatedEvent::dispatch( $session->table );
