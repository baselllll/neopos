 ...
 */
const nsPosDashboardButton = window.nsPosDashboardButton = require('./pages/dashboard/pos/header-buttons/ns-pos-dashboard-button').default;
const nsPosPendingOrderButton = window.nsPosPendingOrderButton = require('./pages/dashboard/pos/header-buttons/ns-pos-' + 'pending-orders' + '-button').default;
const nsPosOrderTypeButton = window.nsPosOrderTypeButton = require('./pages/dashboard/pos/header-buttons/ns-pos-' + 'order-type' + '-button').default;
const nsPosCustomersButton = window.nsPosCustomersButton = require('./pages/dashboard/pos/header-buttons/ns-pos-' + 'customers' + '-button').default;
const nsPosResetButton = window.nsPosResetButton = require('./pages/dashboard/pos/header-buttons/ns-pos-' + 'reset' + '-button').default;
const nsPosCashRegister = window.nsPosCashRegister = require('./pages/dashboard/pos/header-buttons/ns-pos-' + 'registers' + '-button').default;
const nsAlertPopup = window.nsAlertPopup = require('./popups/ns-' + 'alert' + '-popup').default;
const nsConfirmPopup = window.nsConfirmPopup = require('./popups/ns-pos-' + 'confirm' + '-popup').default;
const nsPOSLoadingPopup = window.nsPOSLoadingPopup = require('./popups/ns-pos-' + 'loading' + '-popup').default;
const nsPromptPopup = window.nsPromptPopup = require('./popups/ns-' + 'prompt' + '-popup').default;
const nsLayawayPopup = window.nsLayawayPopup = require('./popups/ns-pos-' + 'layaway' + '-popup').default;
const nsPosShippingPopup = window.nsPosShippingPopup = require('./popups/ns-pos-' + 'shipping' + '-popup').default;
export class POS {
    constructor() {
        this._orderTypeProcessQueue = [];
        this._holdPopupEnabled = true;
        this._initialQueue = [];
    ...
    reset() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.processInitialQueue();
    ...
    initialize() {
        ...
        this._paymentsType = new BehaviorSubject([]);
        this._visibleSection = new BehaviorSubject('both');
        this._options = new BehaviorSubject({});
        this._settings = new BehaviorSubject({});
        this._order = new BehaviorSubject(this.defaultOrder());
        this._selectedPaymentType = new BehaviorSubject(null);
        this._orderTypeProcessQueue = [
            {
    ...
    getWholesalePrice(item, original) {
        if (this.options.getValue().ns_pos_price_with_tax === 'yes') {
    ...
    setHoldPopupEnabled(status = true) {
    ...
    computeProductsTaxes(order) {
        const products = this.products.getValue();
        /**
         * retrieve all products taxes
         * and sum the total.
         */
        const totalInclusiveTax = products.filter(product => product.tax_type === 'inclusive').map((product) => {
            return product.tax_value;
        });
        const totalExclusiveTax = products.filter(product => product.tax_type === 'exclusive').map((product) => {
            return product.tax_value;
        });
        /**
    ...
    canProceedAsLaidAway(_order) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            ...
            let expected = ((_order.total * minimalPaymentPercent) / 100).toFixed(ns.currency.ns_currency_precision);
    ...
    loadOrder(order_id) {
        return new Promise((resolve, reject) => {
            nsHttpClient.get(`/api/nexopos/v4/orders/${order_id}/pos`)
                .subscribe({
                next: (order) => __awaiter(this, void 0, void 0, function* () {
                    /**
                    ...
                    for (let i = 0; i < order.products.length; i++) {
                        ...
                        if (orderProduct.product === null) {
                            orderProduct.product = {
                                ...
                                unit_quantities: [
                                    yield this.defineQuantities(orderProduct)
                                ]
                            };
                        }
                    ...
                    yield this.selectCustomer(order.customer);
                    resolve(order);
                }),
    ...
    checkCart() {
        ...
        unmatchedConditions.forEach(coupon => {
            nsSnackBar.error(__('The coupons "%s" has been removed from the cart, as it\'s required conditions are no more meet.')
                .replace('%s', coupon.name), __('Okay'), {
                duration: 6000
            }).subscribe();
    ...
    refreshCart() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const inclusiveTaxes = products.map((product) => {
            ...
            if (taxType === 'exclusive') {
                order.total = +((order.subtotal + (order.shipping || 0) + tax_value) - order.discount - order.total_coupons).toFixed(ns.currency.ns_currency_precision);
            }
            else {
                order.total = +((order.subtotal + (order.shipping || 0)) - order.discount - order.total_coupons).toFixed(ns.currency.ns_currency_precision);
            }
            this.order.next(order);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.settings.getValue().ns_pos_items_merge) {
                const existing = products.filter(product => {
                     ...
                     */
                    return (product.product_id === cartProduct.product_id &&
    ...
    updateProduct(product, data, index = null) {
         ...
         */
        Vue.set(products, index, Object.assign(Object.assign({}, product), data));
        this.recomputeProducts(products);
        this.products.next(products);
        nsHooks.doAction('ns-after-cart-changed');
    }
    ...
    proceedProductTaxComputation(product, price) {
    ...
    computeCustomProductTax(product) {
        const originalProduct = product.$original();
    ...
    computeNormalProductTax(product) {
        const originalProduct = product.$original();
        const quantities = product.$quantities();
    ...
    computeWholesaleProductTax(product) {
        const originalProduct = product.$original();
        const quantities = product.$quantities();
...
window.POS = new POS;
window.POSClass = POS;
export const POSInit = window.POS;
