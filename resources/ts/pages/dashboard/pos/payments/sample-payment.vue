<template>
    <div class="h-full w-full pt-2">
        <div class="pb-5" v-if="order">
            <div class="grid grid-cols-4 gap-4 mb-4">
                <div id="details" class="flex justify-between items-center payment-card payment-info">
                    <span class="payment-card-text">{{ __( 'Total' ) }} : </span>
                    <span>{{ order.total | currency }}</span>
                </div>
                <div id="discount" @click="toggleDiscount()" class="cursor-pointer flex justify-between items-center payment-card payment-red">
                    <span class="payment-card-text">{{ __( 'Discount' ) }} : </span>
                    <span>{{ order.discount | currency }}</span>
                </div>
                <div id="paid" class="flex justify-between items-center payment-card payment-success">
                    <span class="payment-card-text">{{ __( 'Paid' ) }} : </span>
                    <span>{{ order.tendered | currency }}</span>
                </div>
                <div id="change" class=" flex justify-between items-center payment-card payment-yellow">
                    <span class="payment-card-text">{{ __( 'Change' ) }} : </span>
                    <span>{{ order.change | currency }}</span>
                </div>
            </div>
            <div id="change" class="col-span-2 flex screen-background justify-between items-center">
                <span class="screen-text">{{ __( 'Screen' ) }} : </span>
                <span class="screen-price">{{ backValue / number | currency }}</span>
            </div>
        </div>

            <div class="flex flex-wrap keypad-border">
                <div class="flex-auto">
                    <div id="numpad" class="grid grid-flow-row grid-cols-3 gap-4 grid-rows-3" style="padding: 1px">
                        <div
                            @click="inputValue( key )"
                            :key="index"
                            v-for="(key,index) of keys"
                            style="margin:-1px;"
                            class="h-16 flex ns-keypad-box items-center justify-center cursor-pointer">
                            <span v-if="key.value !== undefined">{{ key.value }}</span>
                            <i v-if="key.icon" class="las text-black" :class="key.icon"></i>
                        </div>
                        <div
                            @click="makeFullPayment()"
                            class="green-button col-span-3 mt-3 flex items-center justify-center cursor-pointer">
                            {{ __( 'Full Payment' ) }}</div>
                    </div>
                </div>
