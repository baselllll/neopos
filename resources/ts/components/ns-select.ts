...
const nsSelect      =   Vue.component( 'ns-select', {
    ...
    <div class="flex flex-col flex-auto ns-select">
        ...
        <div :class="hasError ? 'has-error' : 'is-pristine'" class="border-2 mt-1 relative rounded-md shadow-sm mb-2 overflow-hidden">
            <select style="color: rgb(0 0 0);" :disabled="field.disabled ? field.disabled : false" @change="$emit( 'change', $event )" :name="field.name" v-model="field.value" :class="inputClass" class="form-input block w-full pl-7 pr-12 sm:text-sm sm:leading-5 h-10 appearance-none">
                <option style="color: rgb(0 0 0);" :value="null">{{ __( 'Choose an option' ) }}</option>
                <option  style="color: rgb(0 0 0);" :value="option.value" v-for="option of field.options" class="py-2">{{ option.label }}</option>
            </select>
        </div>
